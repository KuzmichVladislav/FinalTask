...
public class ClientDaoImpl implements ClientDao {
    ...
    public boolean verification(int clientId) throws DaoException {
        ...
        try (Connection connection = ConnectionPool.getInstance().takeConnection();
            ...
            logger.debug(isUpdated ? "Client with id " + clientId + " was verified"
                    : "Client id " + clientId + " verification failed");
    ...
    public List<Client> findAllClientByAnthroponym(String name, String surname) throws DaoException {
        ...
        try (Connection connection = ConnectionPool.getInstance().takeConnection();
            ...
            logger.debug(resultArray.isEmpty() ? "No client with anthroponym " + name + " " + surname + " found"
                    : "Found " + resultArray.size() + " clients with anthroponym " + name + " " + surname);
    ...
    private Client getClientFromResultSet(ResultSet resultSet) throws SQLException {
        return new Client.Builder().id(resultSet.getInt(CLIENT_ID)).login(resultSet.getString(USER_LOGIN))
                ...
                .discountLevel(resultSet.getInt(DISCOUNT_TYPE)).money(resultSet.getBigDecimal(MONEY))
